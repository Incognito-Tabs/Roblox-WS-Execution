...
App.ws("/", (WS, Request) => {
    setTimeout(() => {
        if (Connections.some(Connection => Connection.WS === WS))
            return;
        WS.close();
        vscode_1.default.window.showInformationMessage(`Connected user failed to authenticate, Closing connection..`);
    }, 500);
    WS.on("message", (Unparsed) => {
        ...
        if (Parsed.Method === "Authorization") {
            ...
            if (Check) {
                vscode_1.default.window.showInformationMessage(`Updated WS for ${Parsed.Name}.`);
                return Check.WS = WS;
            }
            vscode_1.default.window.showInformationMessage(`User ${Parsed.Name} Connected.`);
